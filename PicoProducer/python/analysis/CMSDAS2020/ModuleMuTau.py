...
#      veto_electron = False # TODO section 4: introduce a veto electron selection here
      veto_electron = electron.pt > 15.0 and electron.mvaFall17V2noIso_WPL and  electron.pfRelIso03_all < 0.5
      if veto_electron:
    ...
    for jet in Collection(event,'Jet'):
      ...
      if veto_jet and jet.DeltaR(muon)>=0.5 and jet.DeltaR(tau)>=0.5:
        veto_basic_jet = jet.pt > 30.0 
        if veto_basic_jet:
          jets.append(jet)
#          self.jetnumber[0]+=1
#          if jet.pt > self.jet_leading_pt[0] :
#            self.jet_leading_pt[0]=jet.pt
#            self.jet_leading_eta[0]=jet.eta
#          if jet.pt > self.jet_sub_pt[0] and jet.pt <= self.jet_leading_pt[0] and self.jetnumber[0]>1:
...
#            self.jet_sub_eta[0]=jet.eta
        ...
        if veto_bjet:
          bjets.append(jet)
#          self.bjetnumber[0]+=1
#          if jet.pt > self.bjet_leading_pt[0] :
#            self.bjet_leading_pt[0]=jet.pt
#            self.bjet_leading_eta[0]=jet.eta
#          if jet.pt > self.bjet_sub_pt[0] and jet.pt <= self.bjet_leading_pt[0] and self.bjetnumber[0]>1:
#            self.bjet_sub_pt[0]=jet.pt
#            self.bjet_sub_eta[0]=jet.eta
...
######### add high level variables #########
    self.z_vis_pt[0]    = (muon.p4()+tau.p4()).Pt()
    self.z_full_pt[0]   = (puppimet.p4()+muon.p4()+tau.p4()).Pt()
    ...
    bisec = (muon.phi + tau.phi)/2.0
    self.miss_Dzeta[0]  = puppimet.pt * math.cos(bisec - puppimet.phi)
    self.vis_Dzeta[0]   = muon.pt*math.cos(bisec - muon.phi) + tau.pt*math.cos(bisec - muon.phi)
